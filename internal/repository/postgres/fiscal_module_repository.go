// internal/repository/postgres/fiscal_module_repository.go
...
func (r *FiscalModuleRepository) Create(ctx context.Context, module *models.FiscalModule) error {
	query := `
		INSERT INTO fiscal_modules (factory_number, fiscal_number, user_id)
		VALUES ($1, $2, $3)
		RETURNING id, created_at, updated_at`

	err := r.db.QueryRowContext(ctx, query,
		module.FactoryNumber, module.FiscalNumber, module.UserID,
...
func (r *FiscalModuleRepository) GetByID(ctx context.Context, id int) (*models.FiscalModule, error) {
	query := `
		SELECT id, factory_number, fiscal_number, user_id, created_at, updated_at
		FROM fiscal_modules
		WHERE id = $1`

	var module models.FiscalModule
	err := r.db.QueryRowContext(ctx, query, id).Scan(
		&module.ID, &module.FactoryNumber, &module.FiscalNumber,
...
func (r *FiscalModuleRepository) GetByFiscalNumber(ctx context.Context, fiscalNumber string) (*models.FiscalModule, error) {
	query := `
		SELECT id, factory_number, fiscal_number, user_id, created_at, updated_at
		FROM fiscal_modules
	...
	var module models.FiscalModule
	err := r.db.QueryRowContext(ctx, query, fiscalNumber).Scan(
		&module.ID, &module.FactoryNumber, &module.FiscalNumber,
		&module.UserID, &module.CreatedAt, &module.UpdatedAt,
	)
...
func (r *FiscalModuleRepository) Update(ctx context.Context, module *models.FiscalModule) error {
	query := `
		...
		SET factory_number = $1, fiscal_number = $2, user_id = $3, updated_at = NOW()
		WHERE id = $4`

	_, err := r.db.ExecContext(ctx, query,
		module.FactoryNumber, module.FiscalNumber, module.UserID, module.ID,
	)
